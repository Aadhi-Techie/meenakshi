...
export default function Services({  t }) {
  ...
  // சர்வீஸ் லிஸ்ட்
  const servicesList = [
    { id: 'glass', name: isTamil ? "பிரီமியம் கண்ணாடி" : "Premium Glass Supply", desc: isTamil ? "பெரம்பூரில் தரமான கண்ணாடி வேலைகள்." : "High-quality toughened glass solutions in Perambur.", img: "https://images.unsplash.com/photo-1509644851169-2acc08aa25b5?auto=format&fit=crop&q=80&w=400&h=400" },
  ...
  return (
    <section id="services" aria-labelledby="services-heading" style={{ padding: "80px 24px", background: "var(--bg)", position: "relative" }}>
      ...
      <div className="wrap" style={{ display: "flex", flexDirection: "column", gap: 48 }}>
        ...
        {/* சர்வீஸ் கார்டுகள் கிரிட் */}
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(300px, 1fr))", gap: 24, width: "100%" }}>
          {servicesList.map(p => (
            <div key={p.id} role="button" tabIndex={0} onClick={() => handleCardClick(p.id, p.name)} style={{ background: "rgba(255,255,255,0.02)", border: "1px solid var(--brd)", padding: 28, borderRadius: 20, display: "flex", flexDirection: "column", gap: 20, cursor: "pointer", transition: "all 0.3s" }}>
      ...
      {modalOpen && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.85)', backdropFilter: 'blur(8px)', zIndex: 10000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
          <div style={{ background: '#121212', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 24, width: '100%', maxWidth: 850, maxHeight: '85vh', display: 'flex', flexDirection: 'column', position: 'relative', overflow: 'hidden', boxShadow: '0 24px 48px rgba(0,0,0,0.5)' }}>
              ...
              {loadingMedia ? (
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: 16 }}>
                  {galleryItems.map(item => (
                    <div key={item.id} style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.05)', borderRadius: 14, overflow: 'hidden', position: 'relative' }}>
                      <div style={{ height: 160, width: '100%', background: '#000' }}>
                        {item.media_type === 'video' ? (
                          <video src={item.media_url} controls style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                        ) : (
                          <img src={item.media_url} alt={item.title || "Sree Meenakshi Site Work"} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                        )}
                      </div>
                      {item.title && (
                        <div style={{ padding: 12, display: 'flex', alignItems: 'center', gap: 6 }}>
                          {item.media_type === 'video' ? <Video size={14} color="var(--o)" /> : <ImageIcon size={14} color="var(--o)" />}
